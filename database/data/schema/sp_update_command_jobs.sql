...
BEGIN
    ...
    IF sp_status = 'success' THEN
        -- update the job
        update command_jobs
        set status = 'success', modify_timestamp = now(), modify_user = sp_user
        ...
        IF code = '00000' THEN
            ...
            -- insert a log entry
            set v_message = (SELECT CONCAT(vc_command_name, '#', sp_status));
            call sp_insert_log_entry('', 27, v_message, sp_status, sp_user);
        ELSE
            -- insert failed
            set sp_result = 0;
    ...
    ELSEIF sp_status = 'failed' THEN
        ...
        -- update the job 
        update command_jobs
        set status = 'failed', modify_timstamp = now(), modify_user = sp_user
        where idcommand_jobs = sp_idcommand_jobs;
        ...
        IF code = '00000' THEN
            ...
            call sp_insert_log_entry('', 27, vc_command_name, 'success', sp_user);
    ...
    ELSE
        ...
        -- update the job 
        update command_jobs
        set status = 'pending', modify_timstamp = now(), modify_user = sp_user
        where idcommand_jobs = sp_idcommand_jobs;
        ...
        IF code = '00000' THEN
            ...
            -- job pending
            set v_message = (SELECT CONCAT(c_command_name, '#', vdevice_name));
